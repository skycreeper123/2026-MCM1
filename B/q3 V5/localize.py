...
def score_dynamic_detection_points(vertices, current_position, used_positions,
                                   anchor_bearing_deg, route_target, q2_config,
                                   completion_config, deadline_monotonic,
                                   existing_positions=(), candidate_limit=4):
    """Score a bounded set on the current-to-next-entry segment, without truth.

    Every point uses the same guaranteed reception and continuous response
    completion bound as static candidates. The route is a candidate hint only;
    continuation costs are deliberately excluded so cached rows can be repriced.
    """
    current = np.asarray(current_position, dtype=float)
    points = [current]
    if route_target is not None:
        target = np.asarray(route_target, dtype=float)
        points.extend(current + fraction * (target - current)
                      for fraction in (0.25, 0.5, 0.75))
    safe_radius = q2_config.minimum_reception_radius_m - q2_config.safety_margin_m
    excluded = list(used_positions) + list(existing_positions)
    scores = []
    for candidate in points[:max(0, candidate_limit)]:
        if time.monotonic() >= deadline_monotonic:
            break
        point = tuple(map(float, candidate))
        if not _unused(point, excluded, q2_config.repeated_position_tolerance_m) \
                or not is_safe_candidate(candidate, vertices, safe_radius):
            continue
        excluded.append(point)
        bound = completion_upper_bound(
            vertices, point, None, completion_config, current_position,
            completion_config.angle_half_width_deg, anchor_bearing_deg,
            deadline_monotonic,
        )
        if bound is None:
            continue
        ...
        scores.append({
            ...
            "movement_time_s": move_distance / completion_config.speed_mps,
            "objective_s": bound["completion_upper_s"],
