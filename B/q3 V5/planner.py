...
from .localize import (
    ...
    reprice_cover_plan,
    plan_route_distance,
    score_dynamic_detection_points,
)
...
SUPPORTED_STRATEGIES = (
    "b0_serial", "b0_batch_fifo", "v2_local", "v3_global", "v5_dynamic",
)


@dataclass(frozen=True)
class Q3Config:
    strategy: str = "v5_dynamic"
    channel_min: int = 1
    channel_max: int = 20
    ...
    def __post_init__(self):
        ...
        global_mode = self.strategy in ("v3_global", "v5_dynamic")
        cooperative_mode = self.strategy == "v5_dynamic"
        for name, value in {
...
class ChannelRecord:
    ...
    dynamic_detection_scores: list[dict] = field(default_factory=list)
    dynamic_score_origin: tuple[float, float] | None = None
    dynamic_route_target: tuple[float, float] | None = None
...
class Q3Planner:
    ...
    def _mark_upper_bound_absences(self):
        if self.config.strategy in ("v3_global", "v5_dynamic"):
    ...
    def _global_detection_tasks(self, record, continuation, deadline):
        if record.near_position is not None or record.outer_vertices is None \
                ...
                or (
                    self.config.strategy != "v5_dynamic"
        ...
        if record.detection_score_version != record.region_version:
            ...
            record.dynamic_detection_scores = []
            record.dynamic_score_origin = None
            record.dynamic_route_target = None
        ...
        if self.config.strategy == "v5_dynamic":
            # Refresh geometry-dependent rows only after meaningful robot/route
            # motion. A fixed four-row cache cannot grow with scheduling calls.
            target = self._global_continuation(record)
            old_target = record.dynamic_route_target
            route_changed = ((target is None) != (old_target is None)) or (
                target is not None and old_target is not None
                and distance(target, old_target) >= 25.0
            )
            moved = record.dynamic_score_origin is None or distance(
                self.current_position, record.dynamic_score_origin) >= 25.0
            if moved or route_changed:
                now = time.monotonic()
                remaining = self.config.total_planning_time_limit_s - self.planning_time_s
                end = min(now + 0.2,
                          call_started + self.config.q2_calculation_time_limit_s,
                          call_started + max(0.0, remaining))
                if deadline is not None:
                    end = min(end, deadline)
                if now < end:
                    existing = [(row["position"]["x"], row["position"]["y"])
                                for row in record.detection_scores or []]
                    record.dynamic_detection_scores = score_dynamic_detection_points(
                        record.outer_vertices, self.current_position, record.measured_positions,
                        record.anchor_bearing_deg, target, self._q2_config(), self.config,
                        end, existing_positions=existing,
                    )
                    record.dynamic_score_origin = tuple(self.current_position)
                    record.dynamic_route_target = target
        tasks = []
        score_rows = list(record.detection_scores or [])
        if self.config.strategy == "v5_dynamic":
            score_rows.extend(record.dynamic_detection_scores)
    ...
    def _select_global_task(self, deadline):
        tasks = []
        cooperative = self.config.strategy == "v5_dynamic"
        ...
        if task.kind == "LOCALIZE":
            ...
            return Q3Action("MEASURE", task.position, task.channel, "GLOBAL_SAFE_LOCALIZATION",
                            ...
                            decision_budget={"completion_upper_s": task.completion_upper_s,
                                             **({"candidate_origin": task.score.get("candidate_origin", "static")}
                                                if self.config.strategy == "v5_dynamic" else {}),
                                             ...
                                             "selection_mode":
                                                 "v5_dynamic_completion_upper_bound"
                                                 if self.config.strategy == "v5_dynamic"
    ...
    def apply_measure_result(self, action, result, svd_deg=None):
        ...
        if purpose == MeasurePurpose.COVERAGE_SCAN.value:
            ...
            if result == "no_signal":
                ...
                if result == "direction":
                    if self.config.strategy in ("v2_local", "v3_global", "v5_dynamic"):
                        try:
                    ...
                    if self.config.strategy == "b0_serial":
                        ...
                        self._activate_fallback(record)
                    elif self.config.strategy in ("v3_global", "v5_dynamic"):
...
def q3_global_upper_bounds(config=None):
    ...
    return {"strategy_scope": "v3_or_v5_global_finite_loose_bound_not_performance_prediction",
