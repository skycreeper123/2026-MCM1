...
def run_case(case, config, keep_trace=False):
    ...
    for index in range(10000):
        ...
        if action.kind == "MEASURE":
            ...
            if config.strategy in ("v3_global", "v5_dynamic"):
...
def configurations(matrix=False):
    ...
    variants = {
        ...
        "v5_dynamic": Q3Config(strategy="v5_dynamic"),
    }
    if matrix:
        for field, values in {
...
def main(argv=None):
    ...
    if args.output is None:
        args.output = Path(__file__).with_name("validation_results_v5")
        if args.matrix:
    ...
    old, new = summaries["v3_global"], summaries["v5_dynamic"]
    time_metrics = (
    ...
    result = {"algorithm_version": 5, "python": platform.python_version(),
