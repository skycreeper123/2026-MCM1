...
def word_pdf(doc_path,user_profile,convert_tmp_dir,stem,verbose):
    target=Path(convert_tmp_dir)/(stem+'.pdf');shutil.copy2(HERE/'Q2-preview.pdf',target)
...
paths=renderer.rasterize(str(HERE.parent/'Q2正文.docx'),str(HERE/'rendered'),150,False,False)
